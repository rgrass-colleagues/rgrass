...
@stop
@section('content')
<form action="/rgrassAdmin/doAddMainDiscuss" method="post">
    选择评论用户:<br><select name="user_id" id="">
        @foreach($user_info as $v)
        <option value="{{$v->user_id}}">{{$v->username}}</option>
        @endforeach
    </select><br>
    评论标题:<br><input type="text" name="estimate_title"/><br>
    评论内容:<br><textarea name="estimate_content" id="" cols="30" rows="10"></textarea><br>
    选择评价等级:<br><select name="nice_discuss" id="">
        <option value="0">普通评价</option>
        <option value="1">好评</option>
        <option value="2">精华评价</option>
        <option value="3">绝世好评</option>
    </select><br>
    是否置顶:<br><select name="top" id="">
