...
<table class="table table-hover">
    ...
    <tr>
        ...
        <td>
            <a href="/rgrassAdmin/ModifyFlashIndex?id={{$v->id}}" class="btn blue">修改</a>
            @if($v->state==0)
            @if($count>=7)
            <a href="#" onclick="alert('处于显示状态的不能大于7个')" class="btn green">显示</a>
            @else
            <a href="/rgrassAdmin/doChangeState?state=show&&id={{$v->id}}&&redirect=flashIndex" class="btn green">显示</a>
            @endif
            @else
            <a href="/rgrassAdmin/doChangeState?state=hide&&id={{$v->id}}&&redirect=flashIndex" class="btn gray">隐藏</a>
            @endif
        </td>
    </tr>
    @endforeach
